...
const pixivSearchCache = new Map();
...
async function handlePixivCommand(sock, msg) {
    try {
        ...
        if (messageText.toLowerCase().trim() === 'next') {
            ...
            const cachedData = pixivSearchCache.get(cacheKey);

            // Hanya handle jika cache pixiv18 ada dan lebih baru dari cache pixiv SFW
            if (cachedData) {
                    ...
                    if (randomResult.imageUrl) {
                        try {
                            ...
                            if (cachedData) {
                                ...
                                pixivSearchCache.set(cacheKey, cachedData);
                            }
                        } catch (imageError) {
...
async function handleNextPixiv(sock, msg) {
    try {
        ...
        if (cacheAge > 30 * 60 * 1000) {
            pixivSearchCache.delete(cacheKey);
            await sendQuotedMessage(sock, msg.key.remoteJid, 
...
setInterval(() => {
    const now = Date.now();
    for (const [key, data] of pixivSearchCache.entries()) {
        if (now - data.timestamp > 60 * 60 * 1000) { // 1 jam
            pixivSearchCache.delete(key);
        }
    }
