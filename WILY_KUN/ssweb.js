...
• Type "desktop" untuk tampilan normal website`;
            ...
            await ReplyRynzz(sock, msg, helpText);
            return;
        }
...
${config.bot.prefix}ssweb https://youtube.com mobile`;
            ...
            const { ReplyRynzz } = require('../MENU/MENU.js');
            await ReplyRynzz(sock, msg, errorText);
            return;
        }
...
${config.bot.prefix}ssweb ${url} full`;
            ...
            const { ReplyRynzz } = require('../MENU/MENU.js');
            await ReplyRynzz(sock, msg, typeErrorText);
            return;
        }
...
⏳ Mohon tunggu, sedang mengambil screenshot website...`;
        ...
        const { ReplyRynzz } = require('../MENU/MENU.js');
        await ReplyRynzz(sock, msg, loadingText);
...
🤖 Screenshot service memerlukan koneksi stabil`;
        ...
        const { ReplyRynzz } = require('../MENU/MENU.js');
        await ReplyRynzz(sock, msg, criticalErrorText);
    }
}
