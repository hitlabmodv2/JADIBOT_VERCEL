...
async function ReplyRynzz(teks, msg, sock) {
    const packname = "Channel Info System";
    ...
    try {
        ...
        profilePic = "https://files.catbox.moe/9cq0yk.jpg";
    }

    // Get random video/GIF from VID_GIF_ANIME folder
    const videoFolder = path.join(process.cwd(), 'VID_GIF_ANIME');
    let selectedVideoPath = null;
    let selectedFileName = 'kanna-channel';

    // Check if folder exists and get random file
    ...
    const localVideoPath = selectedVideoPath || path.join(process.cwd(), 'VID_GIF_ANIME/kanna-cry.mp4');

    // Check if selected file exists and send as animated video
    if (fs.existsSync(localVideoPath)) {
        const animatedContent = {
            ...
            contextInfo: {
                ...
                forwardedNewsletterMessageInfo: {
                    newsletterName: `📺 ${selectedFileName.charAt(0).toUpperCase() + selectedFileName.slice(1)} • Channel Info`,
                    newsletterJid: "120363312297133690@newsletter",
                },
                externalAdReply: {
                    showAdAttribution: true,
                    title: `📺 ${senderName}`,
                    body: `Channel Info • ${selectedFileName} • ${formattedDate}`,
                    previewType: "VIDEO",
                    thumbnailUrl: profilePic || "https://files.catbox.moe/9cq0yk.jpg",
        ...
        try {
            return await sock.sendMessage(msg.key.remoteJid, animatedContent, { quoted: msg });
        } catch (error) {
    ...
    const content = {
        ...
        contextInfo: {
            ...
            externalAdReply: {
                showAdAttribution: true,
                title: `📺 ${senderName}`,
                body: `Channel Info • ${formattedDate}`,
                previewType: "IMAGE",
                thumbnailUrl: profilePic || "https://files.catbox.moe/9cq0yk.jpg",
                sourceUrl: "https://wa.me/6289688206739",
                mediaType: 1,
...
_❌ Gunakan format URL yang benar!_`;
                ...
                await ReplyRynzz(invalidFormatMessage, m, client);
                return;
            }
...
_📝 Masukkan URL channel untuk melanjutkan!_`;
            ...
            await ReplyRynzz(needUrlMessage, m, client);
            return;
        }
...
_❌ Gunakan invite code yang valid!_`;
                ...
                await ReplyRynzz(invalidCodeMessage, m, client);
                return;
            }
...
_🔥 Newsletter JID: ${newsletterJid}_`;
            ...
            await ReplyRynzz(channelInfoMessage, m, client);
...
_💥 Sistem mengalami kesalahan fatal_`;
        ...
        try {
            await ReplyRynzz(fatalErrorMessage, m, client);
        } catch (replyError) {
            // Silent error handling
        }
    }
