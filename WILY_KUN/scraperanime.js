...
✨ *Semua format gambar didukung termasuk GIF animasi!*`;
        ...
        if (animeCategories.sfw[command] || animeCategories.nsfw[command]) {
            ...
            await ReplyRynzz(`⏳ Sedang mengambil gambar ${command}... Please wait!`, msg, sock);
                    ...
                    while (!sendSuccess && retryCount < maxRetries) {
                        try {
                            if (result.format === 'GIF' || result.isAnimated) {
                                ...
                                await ReplyRynzz(`🖼️ Gambar anime ${command} berhasil dikirim!\n\n✨ Format: ${result.format} (${result.categoryType})\n📏 Size: ${result.sizeFormatted}\n🎌 Source: ${result.source}`, msg, sock);
                            }
                            ...
                            if (retryCount < maxRetries) {
                                await ReplyRynzz(`⚠️ Gagal mengirim ${result.format}, mencoba lagi... (${retryCount}/${maxRetries})`, msg, sock);
                                ...
                                try {
                                    ...
                                    await ReplyRynzz(`📁 ${result.format} anime ${command} berhasil dikirim sebagai dokumen!\n\n✨ Format: ${result.format} (${result.categoryType})\n📏 Size: ${result.sizeFormatted}\n🎌 Source: ${result.source}\n⚠️ Dikirim sebagai file karena error koneksi`, msg, sock);

                                    sendSuccess = true;

                                } catch (docError) {
                                    await ReplyRynzz(`❌ Gagal mengirim ${result.format} ${command} setelah ${maxRetries} percobaan.\n\n🔄 Error: ${sendError.message}\n💡 Coba perintah lagi dalam beberapa saat!`, msg, sock);
                                }
                            }
                        }
                    }
                } else {
                    await ReplyRynzz(`❌ Gagal mengambil gambar ${command}. Silakan coba lagi!`, msg, sock);
                }

            } catch (error) {
                await ReplyRynzz(`❌ Terjadi kesalahan saat mengambil gambar ${command}:\n\n🔄 Error: ${error.message}\n💡 Coba perintah lagi dalam beberapa saat!`, msg, sock);
            }
        }
