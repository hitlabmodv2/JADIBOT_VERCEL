...
// Removed libphonumber-js dependency
...
export default async function serialize(hisoka, msg, store) {
	...
	if (m.isGroup) {
		...
		m.isAdmin = m.isGroup && m.sender && !!m.groupAdmins.find(member => member.id === m.sender);
	...
	if (m.message) {
		...
		if (m.msg?.contextInfo?.quotedMessage) {
			...
			if (m.quoted.message) {
				...
				m.quoted.isBot = m.quoted.id ? (m.quoted.id.startsWith('BAE5') || m.quoted.id.startsWith('HSK')) : false;
				...
				m.quoted.isOwner = m.quoted.sender && config.OWNER.includes(m.quoted.sender.replace(/\D+/g, ''));
			}
		}
	}
