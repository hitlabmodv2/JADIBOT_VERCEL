...
async function handleCommand(sock, msg) {
    try {
        ...
        // Special check untuk next command (pixiv18)
        if (messageText.toLowerCase().trim() === 'next') {
            const { handleNextPixiv } = require('./WILY_KUN/pixiv18.js');
        ...
        // Special handling for image with caption sticker command
        if (msg.message?.imageMessage?.caption) {
            const imageCaption = msg.message.imageMessage.caption.trim();
            if (imageCaption === `${prefix}s` || imageCaption === `${prefix}sticker`) {
                const { handleStickerCommand } = require('./WILY_KUN/sticker.js');
            ...
            // Special handling for removebg with caption
            if (imageCaption === `${prefix}removebg`) {
                const { handleRemoveBgCommand } = require('./WILY_KUN/removebg.js');
                await handleRemoveBgCommand(sock, msg);
                return;
...
📊 *Prefix saat ini:* ${config.bot.prefix}`;
                    ...
                    if (saveConfig(config)) {
                        ...
                        await sendQuotedMessage(sock, msg.key.remoteJid, `❌ Gagal menyimpan pengaturan prefix`, msg);
                    }
                }
                break;
...
📱 *WhatsApp Web:* Connected ✅`;
            ...
            case 'ttaudio':
                {
                    const { handleTikTokAudioCommand } = require('./WILY_KUN/DOWNLOAD/tiktok.js');
...
├─ ${config.bot.prefix}autoceklis2abuabu <on/off>
