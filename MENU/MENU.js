...
// Fungsi ReplyRynzz untuk tampilan keren
async function ReplyRynzz(sock, msg, teks, profilePic = null, options = {}) {
  ...
  const packname = options.packname || "Menu Bot WilyKun";
  const senderName = msg.pushName || (msg.key.participant || msg.key.remoteJid).split('@')[0];
  const formattedDate = new Date().toLocaleDateString('id-ID');
  const useRandomVideo = options.useRandomVideo || false;
  ...
  // Get random video/GIF from VID_GIF_ANIME folder if requested
  if (useRandomVideo) {
    ...
    // Use selected random video or fallback
    const localVideoPath = selectedVideoPath || path.join(__dirname, '../VID_GIF_ANIME/kanna-album.mp4');

    // Check if selected file exists and send as video/GIF
    if (fs.existsSync(localVideoPath)) {
      try {
        const animatedContent = {
          ...
          contextInfo: {
            ...
            forwardedNewsletterMessageInfo: {
              newsletterName: `🎞️ ${selectedFileName.charAt(0).toUpperCase() + selectedFileName.slice(1)} Menu`,
              newsletterJid: "120363312297133690@newsletter",
            },
            externalAdReply: {
              showAdAttribution: true,
              title: `║ 🎞️ ${senderName}`,
              body: `║ Menu • ${selectedFileName} • ${formattedDate}`,
              previewType: "VIDEO",
        ...
        return await sock.sendMessage(msg.key.remoteJid, animatedContent, {
          quoted: msg,
        });
      } catch (error) {
        // Fallback to URL method if local file fails
        try {
          const fallbackAnimated = {
            ...
            contextInfo: {
              ...
              isForwarded: true,
              forwardedNewsletterMessageInfo: {
                newsletterName: "🎞️ Menu System",
                newsletterJid: "120363312297133690@newsletter",
              },
              externalAdReply: {
                showAdAttribution: true,
                title: `🎞️ ${senderName}`,
                body: `Menu • ${formattedDate}`,
                previewType: "VIDEO",
                thumbnailUrl: profilePic || "https://files.catbox.moe/mxohav.gif",
                sourceUrl: "https://wa.me/6289688206739",
                mediaType: 2,
          ...
          return await sock.sendMessage(msg.key.remoteJid, fallbackAnimated, { quoted: msg });
        } catch (fallbackError) {
          // Continue to regular text fallback
        }
      }
    }
  }

  // Regular image method or fallback
  if (profilePic && !useRandomVideo) {
    try {
      const content = {
      ...
      console.log('Failed to send with image, trying fallback');
    }
  }
...
→ ${config.bot.prefix}system
→ ${config.bot.prefix}autoreaction
→ ${config.bot.prefix}tools
→ ${config.bot.prefix}download
→ ${config.bot.prefix}anime
→ ${config.bot.prefix}info
`;

        await ReplyRynzz(sock, msg, menuText, null, { useRandomVideo: true });
...
`;
        ...
        await ReplyRynzz(sock, msg, ownerMenuText, null, { useRandomVideo: true });
    } catch (error) {
...
→ ${config.bot.prefix}antidel <on/off>
...
`;
        ...
        await ReplyRynzz(sock, msg, systemMenuText, null, { useRandomVideo: true });
    } catch (error) {
...
[ FAKE GENERATOR ]
→ ${config.bot.prefix}fakechwa <nama|deskripsi|pengikut|verified>
...
`;
        ...
        await ReplyRynzz(sock, msg, toolsMenuText, null, { useRandomVideo: true });
    } catch (error) {
...
`;
        ...
        await ReplyRynzz(sock, msg, downloadMenuText, null, { useRandomVideo: true });
    } catch (error) {
...
`;
        ...
        await ReplyRynzz(sock, msg, infoMenuText, null, { useRandomVideo: true });
    } catch (error) {
...
`;
        ...
        await ReplyRynzz(sock, msg, autoReactionMenuText, null, { useRandomVideo: true });
    } catch (error) {
...
`;
        ...
        await ReplyRynzz(sock, msg, animeMenuText, null, { useRandomVideo: true });
    } catch (error) {
...
[ SYSTEM & CONFIG ]
→ ${config.bot.prefix}menusystem
→ ${config.bot.prefix}autotyping <on/off>
→ ${config.bot.prefix}autorecord <on/off>
→ ${config.bot.prefix}online <on/off>
→ ${config.bot.prefix}antidel <on/off/all>
→ ${config.bot.prefix}autoreaction <on/off/random>
→ ${config.bot.prefix}setdelay <detik>
...
`;
        ...
        await ReplyRynzz(sock, msg, menuAllText, null, { useRandomVideo: true });
    } catch (error) {
...
// Handler untuk semua menu command yang dipindah dari Wilykun.js
async function handleAllMenuCommands(sock, msg, config, command) {
    ...
    switch (command) {
        ...
        case 'autoreaction':
            ...
            await handleAutoReactionCommand(sock, msg, config, isOwnerOrBot, sendQuotedMessage, loadConfig, saveConfig);
            break;
        case 'tools':
            await handleToolsMenu(sock, msg, config);
            break;
...
async function handleAutoReactionCommand(sock, msg, config, isOwnerOrBot, sendQuotedMessage, loadConfig, saveConfig) {
